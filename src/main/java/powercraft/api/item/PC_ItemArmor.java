...
public abstract class PC_ItemArmor extends ItemArmor implements PC_IItemInfo {
	...
	public void registerIcons(IIconRegister par1IconRegister) {
		for (int i = 0; i < textureNames.length; i++) {
			icons[i] = par1IconRegister.registerIcon(mod_PowerCraft.MODID + ":" + PC_TextureRegistry.getTextureName(module, textureNames[i]));
		}
	}
	
    @Override
    public String getArmorTexture(ItemStack itemstack, Entity entity, int slot, String type){
    	return mod_PowerCraft.MODID + ":" + PC_TextureRegistry.getPowerCraftImageDir()+PC_TextureRegistry.getTextureName(module, armorTexture);
    }
}
