		...
		if (!worldObj.isRemote) {
			if (finished) {
				if (inventoryContents[0] != null) {
					 ...
					 * if(free>0){
					 * 
					 * float outRank = PCma_ItemRanking.getRank(new
					 * PC_ItemStack(inventoryContents[0])); if(outRank>0){ for(int i=11; i<23; i++){
					 * if(inventoryContents[i] != null){
					 * if(inventoryContents[i].isItemEqual(inventoryContents[0])){
					 * sendToOutput(inventoryContents[i]); inventoryContents[i] = null; }else{
					 * if(PCma_ItemRanking.getRank(new PC_ItemStack(inventoryContents[i]))>0){
					 * if(timeCritical){ inventoryContents[9] = inventoryContents[i].copy();
					 * inventoryContents[i] = null; }else{ inventoryContents[9] = decrStackSize(i,
					 * 1); } } if(inventoryContents[9]!=null) break; } } }
					 * 
					 * if(inventoryContents[9] != null){
					 * 
					 * float inRank = PCma_ItemRanking.getRank(new
					 * PC_ItemStack(inventoryContents[9]));
					 * 
					 * if(outRank<=0 || inRank<=0) return;
					 * 
					 * inventoryContents[10] = inventoryContents[0].copy(); inRank *=
					 * inventoryContents[9].stackSize;
					 * 
					 * if(!timeCritical){
					 * 
					 * if(inRank>outRank){ int num = (int)(inRank/outRank); int maxStack =
					 * Math.min(inventoryContents[10].getMaxStackSize(), getInventoryStackLimit());
					 * maxStack = Math.min(free, maxStack); if(num>maxStack) num = maxStack;
					 * inventoryContents[10].stackSize = num; outRank *= num; }
					 * 
					 * }
					 * 
					 * needLoadTime = (int)(outRank / inRank * 20); notifyChanges("needLoadTime");
					 * 
					 * finished = false; } } }
					 */
				}
