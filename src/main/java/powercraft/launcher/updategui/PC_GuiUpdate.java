...
public class PC_GuiUpdate extends GuiScreen {
	...
	private ResourceLocation texture = new ResourceLocation(mod_PowerCraft.MODID + ":" + "/inventory.png");
	...
	public void initGui() {
		...
		buttonList.add(new GuiButton(1, 10, height - 28, 190, 20, StatCollector.translateToLocal("gui.done")));
		buttonList.add(download = new GuiButton(2, 220, height - 58, width - 240, 20,
				StatCollector.translateToLocal("Download")));
		buttonList.add(new GuiButton(3, 220, height - 28, width - 240, 20,
				StatCollector.translateToLocal("Where to watch for downloads")));
		buttonList.add(activate = new GuiButton(4, 220, height - 58, (width - 240) / 2, 20,
				StatCollector.translateToLocal("Activate")));
		buttonList.add(delete = new GuiButton(5, 220 + (width - 240) / 2, height - 58, (width - 240) / 2, 20,
				StatCollector.translateToLocal("Delete")));
		buttonList.add(new GuiButton(6, 10, height - 58, 190, 20, StatCollector.translateToLocal("Ignore")));
	}
	...
	public void drawScreen(int par1, int par2, float par3) {
		...
		// drawCenteredString(fontRenderer, "PowerCraft Update Screen", width / 2, 4,
		// 0xffffff);
	...
	private void drawInfo(String info, boolean highLite) {
		...
		// if(lines.size()==1){
		// fontRenderer.drawStringWithShadow(lines.get(0), width-110, 11,
		// highLite?0xFF4433:0xFFFFFF);
		// }else if(lines.size()>1){
		// fontRenderer.drawStringWithShadow(lines.get(0), width-110, 6,
		// highLite?0xFF4433:0xFFFFFF);
		// fontRenderer.drawStringWithShadow(lines.get(1), width-110, 16,
		// highLite?0xFF4433:0xFFFFFF);
		// }
	}
	...
	public static void show(boolean requestDownloadTarget) {
		...
		// ScaledResolution resolution = new ScaledResolution(smc.gameSettings,
		// smc.displayWidth, smc.displayHeight);
		int width = smc.displayWidth;
		int height = smc.displayHeight;
		gui.setWorldAndResolution(smc, width, height);
		/*while (!stop) {
			if (smc.mcCanvas != null && !smc.isFullScreen()
					&& (smc.mcCanvas.getWidth() != smc.displayWidth || smc.mcCanvas.getHeight() != smc.displayHeight)) {
				...
				GL11.glLoadIdentity();
				GL11.glOrtho(0.0D, resolution.getScaledWidth_double(), resolution.getScaledHeight_double(), 0.0D,
						1000.0D, 3000.0D);
			...
			gui.drawScreen(Mouse.getX() * width / smc.displayWidth,
					height - Mouse.getY() * height / smc.displayHeight - 1, 0.0f);
			Display.update();
			Display.sync(60);
		...
		GL11.glMatrixMode(GL11.GL_PROJECTION);
		GL11.glLoadIdentity();
		GL11.glOrtho(0.0D, resolution.getScaledWidth_double(), resolution.getScaledHeight_double(), 0.0D, 1000.0D,
				3000.0D);
		GL11.glMatrixMode(GL11.GL_MODELVIEW);
		GL11.glLoadIdentity();
		GL11.glTranslatef(0.0F, 0.0F, -2000.0F);
		...
		short var3 = 256;
		short var4 = 256;
		smc.scaledTessellator((resolution.getScaledWidth() - var3) / 2, (resolution.getScaledHeight() - var4) / 2, 0, 0,
				var3, var4);
		GL11.glDisable(GL11.GL_LIGHTING);
		GL11.glDisable(GL11.GL_FOG);
