...
public class PC_TreeHarvesting implements PC_ISpecialHarvesting {
	...
	public void harvestWood(World world, int x, int y, int z, Block block, int meta, int fortune, Tree tree, List<PC_Struct2<PC_VecI, ItemStack>> drops, int recursion){
			...
			if(block!=null){
				...
				while(i.hasNext()){
					...
					for(TreeState sampling:tree.saplings){
						//if(sampling == drop.b && (sampling.metadata == drop.b.getItemDamage() || sampling.metadata==-1)){
	...
	public void harvestLeaves(World world, int x, int y, int z, Block block, int meta, int fortune, Tree tree, List<PC_Struct2<PC_VecI, ItemStack>> drops, int recursion){
		List<ItemStack> blockDrops = PC_BuildingManager.harvestEasy(world, x, y, z, fortune);
		if(blockDrops!=null){
			for(ItemStack blockDrop:blockDrops){
				drops.add(new PC_Struct2<PC_VecI, ItemStack>(new PC_VecI(x, y, z), blockDrop));
