...
public class PC_GuiScrollVersions extends PC_GuiScroll {
	...
	public PC_GuiScrollVersions(int x, int y, int width, int height, PC_GuiScrollModuleAndPackScroll moduleAndPack,
			PC_GuiUpdate guiUpdate) {
		super(x, y, width, height);
		this.moduleAndPack = moduleAndPack;
		this.guiUpdate = guiUpdate;
		if (this.gswidth - 12 > 10) {
			// lText = fontRenderer.listFormattedStringToWidth("Select module or pack",
			// this.gswidth-12);
		}
	}
	...
	public String getObjectString(Object obj, int element) {
		...
		if (obj instanceof XMLPackTag) {
			...
			if (element == activeElement) {
				...
				if (((XMLPackTag) obj).getVersions().get(element).getDownloadLink() == null
						|| ((XMLPackTag) obj).getVersions().get(element).getDownloadLink().equals("")) {
