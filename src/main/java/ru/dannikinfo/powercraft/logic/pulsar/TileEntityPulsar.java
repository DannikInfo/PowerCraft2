...
public class TileEntityPulsar extends TileEntity{
    ...
    public void setTimes(int gdelay, int ghold, World world, int x, int y, int z){
		...
		data.markDirty();
		world.setBlock(x, y, z, BlocksLogic.pulsaroff);
    }

    public boolean isActive()
    {
    	if(worldObj.getBlock(xCoord, yCoord, zCoord) == BlocksLogic.pulsar){
    		paused = false;
    	}else{
    		if(worldObj.getBlock(xCoord, yCoord, zCoord) == BlocksLogic.pulsaroff){
    			paused = true;
    		}
    	}
        return paused;
	...
	public void updateEntity(){
        ...
        {
        	worldObj.setBlock(xCoord, yCoord, zCoord, BlocksLogic.pulsar);
        	delayTimer = -1;
        }
        ...
        {
        	worldObj.setBlock(xCoord, yCoord, zCoord, BlocksLogic.pulsaroff);
        	delayTimer = -1;
        }
	}
    @Override
