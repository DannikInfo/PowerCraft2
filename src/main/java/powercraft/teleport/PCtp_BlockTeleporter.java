	...
	public boolean renderWorldBlock(IBlockAccess world, int x, int y, int z, Object renderer) {
		...
		if (td != null) {
			if (td.direction == PCtp_TeleporterData.N) {
				PC_Utils.setBlockBounds(Blocks.iron_block, m, 0, 0.0625F, n, 0.125F, 0.0625F * 2);
			} else if (td.direction == PCtp_TeleporterData.S) {
				PC_Utils.setBlockBounds(Blocks.iron_block, m, 0, 1 - 0.0625F * 2, n, 0.125F, 1 - 0.0625F);
			} else if (td.direction == PCtp_TeleporterData.E) {
				PC_Utils.setBlockBounds(Blocks.iron_block, 1 - 0.0625F * 2, 0, m, 1 - 0.0625F, 0.125F, n);
			} else if (td.direction == PCtp_TeleporterData.W) {
				PC_Utils.setBlockBounds(Blocks.iron_block, 0.0625F, 0, m, 0.0625F * 2, 0.125F, n);
			}
		}
		PC_Renderer.renderStandardBlock(renderer, Blocks.iron_block, x, y, z);
