...
public class PChg_BlockHologramBlock extends PC_Block {
	 ...
	 * 
	 * Block b = getContainingBlock(world, x, y, z); int meta =
	 * PC_Utils.getMD(world, x, y, z);
	 * 
	 * boolean remove = super.removeBlockByPlayer(world, player, x, y, z);
	 * 
	 * if (remove && !PC_Utils.isCreative(player)){ ItemStack itemStack = new
	 * ItemStack(this); NBTTagCompound nbtTag = itemStack.getTagCompound();
	 * if(nbtTag==null) nbtTag = new NBTTagCompound(); NBTTagCompound nbtTag2 = new
	 * NBTTagCompound(); ItemStack itemBlock;
	 * if(Item.itemsList[b.blockID].getHasSubtypes()){ itemBlock = new ItemStack(b,
	 * 1, meta); }else{ itemBlock = new ItemStack(b); }
	 * itemBlock.writeToNBT(nbtTag2); nbtTag.setCompoundTag("Item", nbtTag2);
	 * itemStack.setTagCompound(nbtTag); dropBlockAsItem_do(world, x, y, z,
	 * itemStack); }
	 * 
	 * return remove; }
	 */
