		...
		if (!entityPlayer.canPlayerEdit(x, y, z, dir, itemStack)) {
			...
			if(y < 255 && y > 0){
				...
				if (placeBlockAt(itemStack, entityPlayer, world, x, y, z, dir, xHit, yHit, zHit, metadata)) {
					//world.playSoundEffect((double) ((float) x + 0.5F), (double) ((float) y + 0.5F), (double) ((float) z + 0.5F), (float) 1F,
					//		(float) (block.stepSound.getVolume() + 1.0F) / 2.0F, (float) block.stepSound.getPitch() * 0.8F);
					--itemStack.stackSize;
				}
			}
