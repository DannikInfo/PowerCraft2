...
public class PCtr_TileEntitySplitter extends PCtr_TileEntitySeparationBeltBase {
	...
	public PCtr_TileEntitySplitter(){
		sepInv = new InventoryBasic("splitter", false, 30);
	}
	...
	protected PC_Direction calculateItemDirection(Entity entity) {
        ...
        for(int s=0; s<6; s++){
        	for(int i=0; i<5; i++){
        		ItemStack stack = sepInv.getStackInSlot(s*5+i);
                if (stack != null
                        && (stack.isItemEqual(itemstack) || (isGroupLogs() && stack.getItem() == Item.getItemFromBlock(Blocks.log) && itemstack.getItem() == Item.getItemFromBlock(Blocks.log))
                                || (isGroupPlanks() && stack.getItem() == Item.getItemFromBlock(Blocks.planks) && itemstack.getItem() == Item.getItemFromBlock(Blocks.planks)) || (isGroupAll() && stack.getItem() == itemstack.getItem())))
                {
