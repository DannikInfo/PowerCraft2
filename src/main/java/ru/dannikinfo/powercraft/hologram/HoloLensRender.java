...
import ru.dannikinfo.powercraft.api.renderer.Renderer;
import ru.dannikinfo.powercraft.api.utils.VecI;

public class HoloLensRender {
	...
	private static void drawArea(EntityPlayer player){
		...
		GL11.glViewport(0, Minecraft.getMinecraft().displayHeight-scale, scale, scale);
		...
		if(update || glList==0){
			...
			RenderBlocks renderer = new HologramRenderBlocks(cc);
			...
			for(int yy=-8; yy<8; yy++){
				for(int xx=-8; xx<8; xx++){
					for(int zz=-8; zz<8; zz++){
						...
						if(block!=null){
							Renderer.renderBlockByRenderType(renderer, block, pos.x+xx, pos.y+yy, pos.z+zz);
						}
					}
				}
			}
