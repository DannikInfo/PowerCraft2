...
import powercraft.teleport.PCtp_BlockTeleporter;
import powercraft.teleport.PCtp_TeleporterData;
import powercraft.teleport.PCtp_TeleporterManager;

public class PC_PacketTeleporterSync extends AbstractServerMessage<PC_PacketTeleporterSync>{
	...
	public PC_PacketTeleporterSync() {}	

	public PC_PacketTeleporterSync(PCtp_TeleporterData td, String target, int code) {
		if(code == 0) {
			this.target = target;
			this.td = td;
		...
		if(side.isServer()){
			if(nbt != null) {
				if(nbt.getInteger("code") == 0) {
					...
					//PC_PacketHandler.sendToAll(new PC_PacketReRender(td.pos, new PC_VecI(td.direction, 0, 0)));
				}else {
					if(nbt.getInteger("code") == 1) {
						PCtp_TeleporterManager tm = new PCtp_TeleporterManager();
						PCtp_TeleporterData td2 = tm.getTargetByName(nbt.getString("target"));
						//PC_PacketHandler.sendToAll(new PC_PacketTeleporterSyncClient(td2, target));
					}
				}
			}
		}
