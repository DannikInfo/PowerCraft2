...
public class PCco_GuiSpawnerEditor implements PC_IGresClient {
	...
	public void initGui(PC_IGresGui gui) {
		...
		Arrays.sort(se,  new Comparator<Entry<String, Class>>(){
			...
			public int compare(Entry<String, Class> arg0, Entry<String, Class> arg1) {
				return PC_LangRegistry.tr(arg0.getKey()).compareToIgnoreCase(PC_LangRegistry.tr(arg1.getKey()));
			}
	...
	public void actionPerformed(PC_GresWidget widget, PC_IGresGui gui) {
		...
		if(id>=0){
			PC_PacketHandler.sendToServer(new PC_PacketSpawnerSet(tems.xCoord, tems.yCoord, tems.zCoord, entityIds.get(id)));
