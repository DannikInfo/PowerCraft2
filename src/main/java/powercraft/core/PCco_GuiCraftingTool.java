...
public class PCco_GuiCraftingTool extends PC_GresBaseWithInventory<PC_TileEntity> implements PC_IGresClient {
	...
	public void initGui(PC_IGresGui gui) {
		...
		for(int y=0; y<5; y++){
			for(int x=0; x<11; x++){
				inv.setSlot(x, y, invSlot.get(y*11+x+30));
			}
		}
		lh.add(inv);
	...
	private void craft(ItemStack p){
		if(p!=null){
			...
			if(p.stackSize>0){
				...
				// TODO: fix it PC_PacketHandler.sendToPacketHandler(true, thePlayer.worldObj, "CraftingToolCrafter", p.itemID, p.getItemDamage());
