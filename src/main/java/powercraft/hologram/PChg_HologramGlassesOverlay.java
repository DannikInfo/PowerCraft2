...
public class PChg_HologramGlassesOverlay implements PC_IOverlayRenderer {
	...
	public static void drawArea(Minecraft mc, EntityPlayer player) {
		...
		ChunkCache cc = new ChunkCache(player.worldObj, pos.x-10, pos.y-10, pos.z-10, pos.x+9, pos.y+9, pos.z+9, 1);
		GL11.glPushAttrib(-1);
				...
				for (int zz = -8; zz < 8; zz++) {
					...
					if (block != null) {
						...
						if (tileEntity != null && !(tileEntity instanceof PChg_TileEntityHologramField) && tileEntity instanceof PC_ITileEntityRenderer) {
							GL11.glPushAttrib(-1);
							PC_TileEntitySpecialRenderer.getInstance().renderTileEntityAt(tileEntity,pos.x+xx, pos.y+yy, pos.z+zz, 10);// = PC_TileEntitySpecialRenderer.getInstance();
							GL11.glPopAttrib();
						}
					}
				}
